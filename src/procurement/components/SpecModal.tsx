        ...
        <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            <div style={{ backgroundColor: 'var(--bg-input)', padding: '12px', borderRadius: '8px' }}>
              ...
              <div style={{ fontSize: '13px', color: 'var(--text-main)', fontWeight: 600 }}>{item.fullSpec.dimensions}</div>
            </div>
            <div style={{ backgroundColor: 'var(--bg-input)', padding: '12px', borderRadius: '8px' }}>
              <div style={{ fontSize: '11px', color: 'var(--text-dim)', marginBottom: '4px' }}>재질 / 소재</div>
              <div style={{ fontSize: '13px', color: 'var(--text-main)', fontWeight: 600 }}>{item.fullSpec.material}</div>
            </div>
            <div style={{ backgroundColor: 'var(--bg-input)', padding: '12px', borderRadius: '8px' }}>
              <div style={{ fontSize: '11px', color: 'var(--text-dim)', marginBottom: '4px' }}>작동 온도 범위</div>
              <div style={{ fontSize: '13px', color: 'var(--text-main)', fontWeight: 600 }}>{item.fullSpec.operatingTemp}</div>
            </div>
            <div style={{ backgroundColor: 'var(--bg-input)', padding: '12px', borderRadius: '8px' }}>
              <div style={{ fontSize: '11px', color: 'var(--text-dim)', marginBottom: '4px' }}>정격 압력 / 등급</div>
              <div style={{ fontSize: '13px', color: 'var(--text-main)', fontWeight: 600 }}>{item.fullSpec.pressureRating}</div>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: 'var(--bg-input)', padding: '14px', borderRadius: '8px' }}>
            ...
            <div style={{ fontSize: '13px', color: 'var(--text-main)', fontWeight: 600, marginBottom: '10px' }}>{item.fullSpec.manufacturer}</div>
